        ...
        <tbody>
            ...
            if($dataProvider) :
                foreach($dataProvider as $row) {
                    $imgPct = ($row->picture_img) ? base_url() . 'uploads/' . $row->picture_img : base_url() . 'assets/img/undraw_profile_1.svg';
                    echo '<tr>
                        <td>'.$row->id.'</td>
                        <td>'.$row->username.'</td>
                        <td>'.$row->nama.'</td>
                        <td>'.$row->email.'</td>
                        <td>'.$row->nohape.'</td>
                        <td><img class="img-account-profile mb-2" src="' . $imgPct . '" width="70" alt=""></td>
